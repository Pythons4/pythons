...
class SignInPage extends React.Component<{}, { value: string; alert: boolean; useremail: string; userpass: string, username: string, userphone: string }>{
    ...
    onSubmit(data: any) {
        ...
        if (validateEmail(this.state.useremail))
            ...
            //check password validation (more than 8 digit and contain small and capital letters)
                ...
                this.setState({
                    value: "Password should contain letters and numbers and should be more than 8 digit",
                    alert: true
                })
            }
            //if all feild are valid send post request to sign up the user
                ...
                store.dispatch(signup({
                    user_name: this.state.username, user_password: this.state.userpass,
                    user_email: this.state.useremail, user_phon: this.state.userphone
                }))
                this.setState({
                    alert: false
                })
    ...
    render() {
        return (
            <div className="d-flex justify-content-center shadow" style={{ borderRadius: '5px', paddingTop: '20px', width: '800px', marginLeft: 'auto', marginRight: 'auto', marginBottom: '30px' }}>
                <div className="backimg2">

                </div>

                <Container maxWidth="xs" style={{ marginRight: '0', marginLeft: '0', marginTop: '35px' }}>
                    ...
                    <p style={{ marginTop: '6px' }}>Already Have an Account? <Link style={{ color: '#3F51B5' }} to={{ pathname: 'signin' }}>Signin</Link></p>
                </Container >
            </div>
        )
    }
